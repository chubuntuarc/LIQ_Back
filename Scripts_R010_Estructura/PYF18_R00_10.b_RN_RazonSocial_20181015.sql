	...
	IF @VP_RESULTADO='WIWI'
		...
		IF @VP_RESULTADO=''
			IF @VP_N_EMPRESA_X_D_EMPRESA>0
				SET @VP_RESULTADO =  'Ya existen [Razones Sociales] con esa [Descripci�n('+@PP_D_RAZON_SOCIAL+')].' 
		END	
...
AS
	...
	IF @VP_RESULTADO=''
		IF @VP_N_UNIDAD_OPERATIVA_X_RAZON_SOCIAL>0
			SET @VP_RESULTADO =  'Existen [Sucursales] asignadas a esta Raz�n Social.' 
...
--	SELECT	@VP_N_EMPRESA
	...
	IF @VP_RESULTADO=''
		IF ( @VP_K_RAZON_SOCIAL IS NULL )
			SET @VP_RESULTADO =  'La [Raz�n Social] no existe.' 
	
	-- ===========================

	IF @VP_RESULTADO=''
		IF @VP_L_BORRADO=1
			SET @VP_RESULTADO =  'La [Raz�n Social] fue dada de baja.' 
